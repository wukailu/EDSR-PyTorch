...
if '/home/kailu/EDSR-PyTorch/code/frameworks/pisr' in sys.path:
    sys.path.remove('/home/kailu/EDSR-PyTorch/code/frameworks/pisr')
...
from frameworks.pisr.utils.config import load
import frameworks.pisr.utils as utils
from frameworks.pisr.datasets import get_train_dataloader, get_valid_dataloader
from frameworks.pisr.losses import get_loss
from frameworks.pisr.models import get_model
from frameworks.pisr.optimizers import get_optimizer
from frameworks.pisr.schedulers import get_scheduler
from frameworks.pisr.transforms import get_transform
from frameworks.pisr.utils.metrics import get_psnr
from frameworks.pisr.utils.utils import quantize
from frameworks.pisr.visualizers import get_visualizer
...
def train(config, student_model, teacher_model, dataloaders, criterion,
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        checkpoint.save_checkpoint(config, student_model, optimizer,
                                         epoch, 0,
                                         model_type='student')
        if psnr > best_psnr:
            best_psnr = psnr
...
def run(config):
    ...
    # for teacher
    optimizer_t = None
    checkpoint_t = checkpoint.get_initial_checkpoint(config,
                                                         model_type='teacher')
    if checkpoint_t is not None:
        last_epoch_t, step_t = checkpoint.load_checkpoint(teacher_model,
    ...
    checkpoint_s = checkpoint.get_initial_checkpoint(config,
                                                         model_type='student')
    if checkpoint_s is not None:
        last_epoch_s, step_s = checkpoint.load_checkpoint(student_model,
