...
from model.basic_cifar_models.utils import register_model, unpack_feature, pack_feature, record_f
    ...
    def __init__(self, num_layers, num_input_features, bn_size, growth_rate, drop_rate):
        ...
        for i in range(num_layers):
            layer = _DenseLayer(num_input_features + i * growth_rate, growth_rate,
                                bn_size, drop_rate)
            self.add_module('denselayer%d' % (i + 1), record_f(layer))
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
        ...
        for i, num_layers in enumerate(block_config):
            ...
            if i != len(block_config) - 1:
                ...
                self.features.add_module('transition%d' % (i + 1), record_f(trans))
                num_features = num_features // 2

        # Final batch norm
        self.features.add_module('norm5', record_f(nn.BatchNorm2d(num_features), record=False))
