#######################################################
##  from https://github.com/ymff0592/super-resolution/
########################################################
...
@register_model
def LatticeNet(**kwargs):
    model = LatticeNet_Model(**kwargs)
    return model
...
class LatticeNet_Model(nn.Module):
    def __init__(self, in_nc=3, n_feats=64, out_nc=3, nDiff=16, scale=4, rgb_range=255, **kwargs):
        super(LatticeNet_Model, self).__init__()
        ...
        modules_tail = [nn.Conv2d(n_feats, n_feats, kernel_size=3, padding=1, bias=True),
                        nn.Conv2d(n_feats, out_nc * (scale ** 2), kernel_size=3, padding=1, bias=True),
