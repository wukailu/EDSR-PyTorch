    ...
    def __init__(self, in_channel, kernel_size=3, scale=2, conv=model.model_utils.default_conv):
        ...
        self.error_encode = nn.Sequential(
            *[nn.ConvTranspose2d(in_channel, in_channel, stride_conv_ksize, stride=stride, padding=padding),
              nn.PReLU()])
        self.post_conv = common.BasicBlock(conv, in_channel, in_channel, kernel_size, stride=1, bias=True,
                                           act=nn.PReLU())
        if scale == 4:
...
class CSNLN_Model(nn.Module):
    def __init__(self, depth=12, rgb_range=255, n_colors=3, n_feats=64, scale=4, conv=model.model_utils.default_conv, **kwargs):
        ...
        # define head module
        m_head = [
            common.BasicBlock(conv, n_colors, n_feats, kernel_size, stride=1, bias=True, bn=False, act=nn.PReLU()),
